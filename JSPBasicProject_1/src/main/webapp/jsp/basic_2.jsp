...
<%--
	...
	1) 웹동작
		...
		=> 반드시 URL을 이용할 때는 
			...
			** 파일의 확장자는 개발자가 설정 할 수 있다
				.회사명 ( .naver, .daum, .do )
		=> 나머지는 반드시 파일명을 첨부한다
		
		http://localhost/JSPBasicProjecct_1/jsp/basic_2.jsp
		--------------------------------------------------- URL
		---------------- ----------------------------------
			서버 정보 						URI
						-------------------
		...
		구성요소
			...
			---
				...
				1. 사용자가 요청한 JSP파일 찾기
					...
					= 있는 경우 
								|
				2. JSP에 해당되는 Java파일이 생성되어있는지 확인
					= 자바파일이 있는 경우 => 수정후에 컴파일
					= 자바 파일이 없는 경우 => 생헝후 컴파일
					a_jsp.java
